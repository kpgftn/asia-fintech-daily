...
keyword topic chips (no LLM — raw links by design), and renders a static page
per person into docs/. GitHub Actions runs this every morning; GitHub Pages
serves the result. No servers, no accounts, no database.
...
def collect_for(cfg: dict) -> dict:
    ...
    for lst in by_jur.values():
        lst.sort(key=lambda x: x["ts"], reverse=True)
    leadership.sort(key=lambda x: x["ts"], reverse=True)
    regional.sort(key=lambda x: x["ts"], reverse=True)

    cap = cfg.get("max_per_jurisdiction", 40)
    return {
        "leadership": leadership[:cap],
        "by_jur": {j: items[:cap] for j, items in by_jur.items()},
        "regional": regional[:cap],
    }
...
def _section(title: str, items: list, cls: str = "") -> str:
    if not items:
        return (f'<h2>{html.escape(title)}</h2>'
                f'<p class="empty">Nothing in the last 48 hours.</p>')
    body = "".join(_item_html(i) for i in items)
    return (f'<h2>{html.escape(title)} <span class="count">{len(items)} items</span></h2>'
...
def render(cfg: dict, data: dict, archive_links: list[str]) -> str:
    ...
    sections = [_section("Leadership changes", data["leadership"], "lead")]
    sections += [_section(j, items) for j, items in data["by_jur"].items()]
    sections.append(_section("Regional & global", data["regional"]))
